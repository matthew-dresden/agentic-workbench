# E9-F1-S1-T04: `mpm remove --help` snapshot
...
Add the `mpm remove --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-remove.txt`:

- Verbatim `mpm remove --help` output as rendered by the argparse
  subparser landed by E2-F5-S1-T1 (`mpm remove` core (accepts source-name OR entry-name)).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-remove", ("remove",), "mpm-remove.txt")`. The row goes after the row
...
Section 14 names `mpm remove --help` per the per-command snapshot rule. The fixture is the verbatim text argparse renders for the remove subparser registered by E2-F5-S1-T1.
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-remove.txt` with
   the verbatim expected stdout for `python -m mpm_cli remove --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-remove", ("remove",), "mpm-remove.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-remove`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-remove.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm remove --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-remove.txt` exists and contains the verbatim `mpm remove --help` output as rendered by the argparse subparser landed by E2-F5-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-remove", ("remove",), "mpm-remove.txt")`, appended after the row added by E9-F1-S1-T03.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-remove` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-remove` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "remove", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-remove.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-remove.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-remove", ("remove",), "mpm-remove.txt")`. No other edits. |
| `tests/fixtures/help/mpm-remove.txt` | New -- verbatim `mpm remove --help` output captured from the argparse subparser landed by E2-F5-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-remove.txt` to the fixture-naming-convention table. |
