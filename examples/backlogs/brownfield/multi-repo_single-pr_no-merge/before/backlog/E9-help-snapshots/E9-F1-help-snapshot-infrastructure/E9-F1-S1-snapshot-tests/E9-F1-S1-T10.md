# E9-F1-S1-T10: `mpm completion --help` snapshot
...
Add the `mpm completion --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-completion.txt`:

- Verbatim `mpm completion --help` output as rendered by the argparse
  subparser landed by E7-F1-S1-T1 (`mpm completion` subcommand wiring (shtab integration)).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-completion", ("completion",), "mpm-completion.txt")`. The row goes after the row
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-completion.txt` with
   the verbatim expected stdout for `python -m mpm_cli completion --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-completion", ("completion",), "mpm-completion.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-completion`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-completion.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm completion --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-completion.txt` exists and contains the verbatim `mpm completion --help` output as rendered by the argparse subparser landed by E7-F1-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-completion", ("completion",), "mpm-completion.txt")`, appended after the row added by E9-F1-S1-T09.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-completion` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-completion` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "completion", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-completion.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-completion.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-completion", ("completion",), "mpm-completion.txt")`. No other edits. |
| `tests/fixtures/help/mpm-completion.txt` | New -- verbatim `mpm completion --help` output captured from the argparse subparser landed by E7-F1-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-completion.txt` to the fixture-naming-convention table. |
