# E9-F1-S1-T07: `mpm install --help` snapshot
...
Add the `mpm install --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-install.txt`:

- Verbatim `mpm install --help` output as rendered by the argparse
  subparser landed by E3-F3-S1-T1 (Lockfile state-matrix branching in `mpm install`).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-install", ("install",), "mpm-install.txt")`. The row goes after the row
...
Section 14 names `mpm install --help` as the stateful-command-shape representative (consumes `.mpm` + `.mpm.lock`, the `--frozen` / `--update` flags per Section 5.6, and the Catalog source group's lockfile-fallback note). The fixture is the verbatim text argparse renders for the install subparser, including the existing `--mpm-file` flag, after the state-matrix branching from E3-F3-S1-T1 lands.
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-install.txt` with
   the verbatim expected stdout for `python -m mpm_cli install --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-install", ("install",), "mpm-install.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-install`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-install.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm install --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-install.txt` exists and contains the verbatim `mpm install --help` output as rendered by the argparse subparser landed by E3-F3-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-install", ("install",), "mpm-install.txt")`, appended after the row added by E9-F1-S1-T06.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-install` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-install` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "install", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-install.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-install.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-install", ("install",), "mpm-install.txt")`. No other edits. |
| `tests/fixtures/help/mpm-install.txt` | New -- verbatim `mpm install --help` output captured from the argparse subparser landed by E3-F3-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-install.txt` to the fixture-naming-convention table. |
