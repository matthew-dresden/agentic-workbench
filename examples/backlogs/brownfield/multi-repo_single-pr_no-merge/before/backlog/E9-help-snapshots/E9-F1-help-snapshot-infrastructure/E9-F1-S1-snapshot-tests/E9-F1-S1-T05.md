# E9-F1-S1-T05: `mpm outdated --help` snapshot
...
Add the `mpm outdated --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-outdated.txt`:

- Verbatim `mpm outdated --help` output as rendered by the argparse
  subparser landed by E4-F1-S1-T1 (`mpm outdated` core (tag-based comparison logic)).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-outdated", ("outdated",), "mpm-outdated.txt")`. The row goes after the row
...
T11) per `workbench/docs/backlog-contract.md` Manifest Conflict Rule
plus the issue #145 N-claimant ordered-chain DAG.

Section 14 names `mpm outdated --help` per the per-command snapshot rule. The fixture covers the `--fail-on-upgrade` opt-in flag (spec Section 0.2 + Section 4.4) and the standard Catalog source group. The fixture is the verbatim text argparse renders for the outdated subparser registered by E4-F1-S1-T1.
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-outdated.txt` with
   the verbatim expected stdout for `python -m mpm_cli outdated --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-outdated", ("outdated",), "mpm-outdated.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-outdated`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-outdated.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm outdated --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-outdated.txt` exists and contains the verbatim `mpm outdated --help` output as rendered by the argparse subparser landed by E4-F1-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-outdated", ("outdated",), "mpm-outdated.txt")`, appended after the row added by E9-F1-S1-T04.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-outdated` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-outdated` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "outdated", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-outdated.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-outdated.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-outdated", ("outdated",), "mpm-outdated.txt")`. No other edits. |
| `tests/fixtures/help/mpm-outdated.txt` | New -- verbatim `mpm outdated --help` output captured from the argparse subparser landed by E4-F1-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-outdated.txt` to the fixture-naming-convention table. |
