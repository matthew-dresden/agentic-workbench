# E9-F1-S1-T08: `mpm doctor --help` snapshot
...
Add the `mpm doctor --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-doctor.txt`:

- Verbatim `mpm doctor --help` output as rendered by the argparse
  subparser landed by E5-F1-S1-T1 (`mpm doctor` consistency checks 1-5).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-doctor", ("doctor",), "mpm-doctor.txt")`. The row goes after the row
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-doctor.txt` with
   the verbatim expected stdout for `python -m mpm_cli doctor --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-doctor", ("doctor",), "mpm-doctor.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-doctor`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-doctor.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm doctor --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-doctor.txt` exists and contains the verbatim `mpm doctor --help` output as rendered by the argparse subparser landed by E5-F1-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-doctor", ("doctor",), "mpm-doctor.txt")`, appended after the row added by E9-F1-S1-T07.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-doctor` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-doctor` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "doctor", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-doctor.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-doctor.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-doctor", ("doctor",), "mpm-doctor.txt")`. No other edits. |
| `tests/fixtures/help/mpm-doctor.txt` | New -- verbatim `mpm doctor --help` output captured from the argparse subparser landed by E5-F1-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-doctor.txt` to the fixture-naming-convention table. |
