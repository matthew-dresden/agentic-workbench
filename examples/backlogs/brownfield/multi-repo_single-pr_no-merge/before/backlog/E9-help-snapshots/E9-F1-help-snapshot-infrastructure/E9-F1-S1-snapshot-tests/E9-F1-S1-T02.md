# E9-F1-S1-T02: `mpm list --help` snapshot
...
Add the `mpm list --help` snapshot to the snapshot harness landed by
...
New fixture file `tests/fixtures/help/mpm-list.txt`:

- Verbatim `mpm list --help` output as rendered by the argparse
  subparser landed by E2-F2-S1-T1 (`mpm list` default output + entry-name index).
...
- Append a single row to the module-level `_HELP_CASES` constant:
  `("mpm-list", ("list",), "mpm-list.txt")`. The row goes after the row
...
Section 14 calls out `mpm list --help` as the representative-shape snapshot covering the Arguments / Filtering / Output / History groups, the mutually-exclusive `--tree` / `--all-versions` callout, and the Catalog source group. The fixture is the verbatim text that argparse renders for the list subparser registered by E2-F2-S1-T1.
...
1. **TDD RED (fixture)**: Author `tests/fixtures/help/mpm-list.txt` with
   the verbatim expected stdout for `python -m mpm_cli list --help`.
...
2. **TDD RED (harness update)**: Append the new parametrise row
   `("mpm-list", ("list",), "mpm-list.txt")` to `_HELP_CASES` in
   `tests/functional/test_help_snapshots.py`. Run
   `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-list`
...
4. **TDD REFACTOR**: Confirm the new row is the ONLY change to
   ...
   line endings via `file --mime-encoding tests/fixtures/help/mpm-list.txt`.
...
- **Migration spec:** `spec/mpm-list-add-lock-features-spec.md` Section 14 (CLI `--help` reference, snapshot-tested) -- the `mpm list --help` per-command snapshot rule.
...
- [ ] AC-FUNC-001 New fixture `tests/fixtures/help/mpm-list.txt` exists and contains the verbatim `mpm list --help` output as rendered by the argparse subparser landed by E2-F2-S1-T1.
...
- [ ] AC-FUNC-003 `tests/functional/test_help_snapshots.py`'s `_HELP_CASES` list contains exactly one new row added by this task: `("mpm-list", ("list",), "mpm-list.txt")`, appended after the row added by E9-F1-S1-T01.
...
- [ ] AC-TEST-001 The functional snapshot test passes for the `mpm-list` case: `uv run pytest tests/functional/test_help_snapshots.py -v -k mpm-list` exits zero and reports `1 passed`.
...
- [ ] AC-CYCLE-001 End-to-end cycle: run `subprocess.run([sys.executable, "-m", "mpm_cli", "list", "--help"], capture_output=True, env=_clean_env())`; assert captured stdout bytes exactly equal the bytes of `tests/fixtures/help/mpm-list.txt` read via `read_bytes()`; assert exit code 0; assert stderr empty. Recorded in `## TDD Cycle Log` with the exact captured stdout length and the first 200 bytes for audit.
- [ ] AC-DOC-001 `tests/fixtures/help/README.md` is updated in the same commit to list the new `mpm-list.txt` fixture in the fixture-naming-convention table (so the README and the on-disk fixture set stay in sync).
...
| File | Change |
|------|--------|
| `tests/functional/test_help_snapshots.py` | Update -- append one row to `_HELP_CASES`: `("mpm-list", ("list",), "mpm-list.txt")`. No other edits. |
| `tests/fixtures/help/mpm-list.txt` | New -- verbatim `mpm list --help` output captured from the argparse subparser landed by E2-F2-S1-T1. |
| `tests/fixtures/help/README.md` | Update -- add `mpm-list.txt` to the fixture-naming-convention table. |
