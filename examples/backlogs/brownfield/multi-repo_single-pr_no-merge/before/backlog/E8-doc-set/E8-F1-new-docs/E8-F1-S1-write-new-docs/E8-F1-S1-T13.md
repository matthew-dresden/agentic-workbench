...
1. **Canonical table** (the four codes spec Section 8 enumerates):
   ...
   - `3` -- deprecated invocation (currently `mpm bootstrap` and
     `mpm bootstrap list`).
   - Future codes reserved.

2. **Cross-reference table:** which subcommands emit which exit codes
   under which conditions. At minimum:
   - `mpm list`, `mpm add`, `mpm remove`, `mpm outdated`,
     `mpm why`, `mpm catalog audit`, `mpm validate <sub>`,
     `mpm install`, `mpm doctor` -- 0 success, 1 hard error, 2
     argparse error.
   - `mpm bootstrap` and `mpm bootstrap list` -- 3 deprecated
...
`docs/exit-codes.md` NEW bullet, and Section 4.9 (`mpm bootstrap`
exit-3 contract).
...
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 8 (the `docs/exit-codes.md` NEW bullet).
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 4.9 (`mpm bootstrap` exit-3 contract).
...
- [ ] AC-FUNC-004 `Per-subcommand reference` H2 contains a Markdown table with columns `Subcommand`, `0`, `1`, `2`, `3` and rows for: `mpm list`, `mpm add`, `mpm remove`, `mpm outdated`, `mpm why`, `mpm catalog audit`, `mpm validate xml`, `mpm validate marketplace`, `mpm validate metadata`, `mpm install`, `mpm doctor`, `mpm bootstrap`, `mpm bootstrap list`, `mpm clean`, `mpm completion`, `mpm repo`.
- [ ] AC-FUNC-005 `mpm bootstrap` and `mpm bootstrap list` rows show exit `3` for any non-`--help` invocation (per spec Section 4.9).
...
| File | Change |
|------|--------|
| `docs/exit-codes.md` | New -- canonical exit-code reference for every `mpm` subcommand. |
...
[2026-05-11 17:21 UTC] [agent/operator] [WU_WIRED] E8-F1-S1-T13 manually blocked on E6-F1-S1-T1 via `workbench add-dep`. [BLOCKED_PENDING_PROPOSAL] E6-F1-S1-T1
