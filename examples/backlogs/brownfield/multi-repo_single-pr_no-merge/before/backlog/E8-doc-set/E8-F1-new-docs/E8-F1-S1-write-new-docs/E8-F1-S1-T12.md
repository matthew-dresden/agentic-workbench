...
2. **What it WILL cover when implemented:** a manifest-repo-declared
   ...
   `<catalog-metadata>` or a top-level `repo-specs/.mpm-catalog-version`
   file); the mpm CLI's supported-version range; the negotiation rule
   (forward-compatible read, hard-error on unsupported version).

3. **Current behaviour:** mpm does NOT inspect a format version today.
   Manifest repos and the mpm CLI evolve together; if a manifest repo
   uses a feature your mpm version does not support, the failure mode
...
Spec source: `spec/mpm-list-add-lock-features-spec.md` Section 15
...
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 15 (future work).
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 8 (the `docs/catalog-format-versioning.md` NEW bullet).
...
- [ ] AC-FUNC-004 `Planned scope` H2 documents the three planned elements: manifest-repo-declared format version, mpm CLI's supported-version range, negotiation rule (forward-compatible read, hard-error on unsupported).
- [ ] AC-FUNC-005 `Current behaviour` H2 explicitly states mpm does NOT inspect a format version today.
