# E8-F2-S1-T3: Add `mpm add` recommendation note to `docs/multi-source-guide.md`
...
note that `mpm add` is the recommended way to produce
`MPM_SOURCE_*` triples (instead of hand-writing them).

The update MUST cover:

1. **Recommendation note:** a clear call-out near the top of the
   existing guide stating that `mpm add <name>@<spec>
   --catalog-source <url>@<ref>` is the preferred way to add a new
   `MPM_SOURCE_<name>_{URL,REVISION,PATH}` triple to `.mpm`.

2. **Why:** explains that `mpm add` handles source-name normalization
...
3. **When to hand-write:** brief note on the edge cases where hand-
   writing remains valid (e.g., specific `MPM_SOURCE_*_PATH`
   override; bare branch pin to a non-`<catalog-metadata>`-tracked
   project).

4. **Cross-link:** to `docs/list-and-add.md` for the full `mpm add`
   reference.
...
`docs/multi-source-guide.md` update bullet, Section 4.2 (`mpm add`).
...
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 8 (the `docs/multi-source-guide.md` update bullet).
- **Spec:** `spec/mpm-list-add-lock-features-spec.md` Section 4.2 (`mpm add`).
...
- [ ] AC-FUNC-002 A recommendation note is inserted near the top of the doc stating `mpm add <name>@<spec> --catalog-source <url>@<ref>` is the preferred way to produce `MPM_SOURCE_*` triples.
...
- [ ] AC-FUNC-005 The note cross-links to `docs/list-and-add.md` (ref) (full `mpm add` reference) and `docs/catalogs-explained.md` (ref) (first-time on-ramp).
...
| File | Change |
|------|--------|
| `docs/multi-source-guide.md` | Update -- add note recommending `mpm add` as the preferred way to produce `MPM_SOURCE_*` triples, with cross-links. |
