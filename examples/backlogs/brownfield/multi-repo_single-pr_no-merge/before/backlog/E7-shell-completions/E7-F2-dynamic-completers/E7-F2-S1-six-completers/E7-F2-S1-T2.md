# E7-F2-S1-T2: `__complete_source_names_in_mpm` dynamic completer
...
Implement the `__complete_source_names_in_mpm` dynamic completer
(Section 11.3 row 2). New hidden subcommand
`mpm __complete_source_names_in_mpm <current-token>` that:

1. Resolves the path to the active `.mpm` file from
   `${MPM_MPM_FILE:-./.mpm}` (the env-var precedence ladder
   already implemented in `core/config.py`; reuse it -- do not
   reimplement).
2. Parses every `MPM_SOURCE_<name>_URL` key in the `.mpm` file
   (the existing parser in `core/mpm_file.py` is the canonical
   reader -- reuse).
3. Emits one normalized source name per line. Normalization is one-way
   ...
   already lives in the `<name>` portion of the `MPM_SOURCE_<name>_URL`
...
`MPM_COMPLETION_ENABLED=0` short-circuit.
...
Used by: `mpm remove <name>`, `mpm install --refresh-lock-source <name>`,
and `mpm why <name>` (resolved against lockfile in a separate
completer). The shell helper invoking this is
`_mpm_complete_source_names_in_mpm` from E7-F1-S1-T2's PREAMBLE.
...
1. **TDD RED:** Author `tests/unit/test_complete_source_names.py`
   with parametrized cases: empty `.mpm` returns empty list;
   `.mpm` with three sources returns those three normalized names;
   prefix filter narrows correctly; malformed `.mpm` (missing
   `MPM_SOURCE_*_URL` key shape) returns empty + log entry;
   `${MPM_MPM_FILE}` pointing at a non-existent file returns
   empty + log entry; `MPM_COMPLETION_ENABLED=0` returns empty
   without reading the file.
2. **TDD RED (integration):** Author
   `tests/integration/test_complete_source_names.py` that builds a
   real `.mpm` fixture file with three sources, sets
   `${MPM_MPM_FILE}` to that path, invokes
   `mpm __complete_source_names_in_mpm ""` via subprocess, and
   asserts stdout is the sorted list of normalized names.
3. **TDD GREEN:** Implement
   `src/mpm_cli/completions/source_names.py` exporting
   `complete(current_token: str) -> list[str]`. Wire the hidden
   subcommand in `cli.py`. Reuse `core/mpm_file.py` for parsing.
...
- **Migration spec:** spec Section 11.3 row 2 -- `__complete_source_names_in_mpm` contract (with normalization-is-lossy note).
...
- [ ] AC-FUNC-001 `mpm __complete_source_names_in_mpm ""` against a `.mpm` fixture with three `MPM_SOURCE_foo_URL`, `MPM_SOURCE_bar_URL`, `MPM_SOURCE_baz_URL` entries prints exactly `foo`, `bar`, `baz` (one per line, sorted).
- [ ] AC-FUNC-002 `mpm __complete_source_names_in_mpm "f"` returns only entries whose name starts with `f`.
...
- [ ] AC-FUNC-004 When `${MPM_MPM_FILE}` points at a non-existent path, the completer prints nothing, exits 0, and writes a structured log line to `${MPM_CACHE_DIR}/completion-errors.log` (format: `<ISO-8601> __complete_source_names_in_mpm FileNotFoundError: <path>`).
- [ ] AC-FUNC-005 Malformed `.mpm` content (no `MPM_SOURCE_*_URL` keys) produces empty stdout AND a structured log entry.
- [ ] AC-FUNC-006 `MPM_COMPLETION_ENABLED=0` causes the completer to print nothing, exit 0, NOT touch the cache, and NOT read the `.mpm` file (verified by a stub that records reads).
- [ ] AC-FUNC-007 The hidden subcommand is registered with `help=argparse.SUPPRESS` and does not appear in `mpm --help` output.
- [ ] AC-TEST-001 `tests/unit/test_complete_source_names.py` parametrized cases cover: happy path, empty `.mpm`, malformed `.mpm`, missing file, prefix filter, `MPM_COMPLETION_ENABLED=0`.
- [ ] AC-TEST-002 `tests/integration/test_complete_source_names.py` builds a real `.mpm` fixture file and exercises the subprocess path end-to-end.
- [ ] AC-CYCLE-001 End-to-end cycle: write a `.mpm` fixture with three sources `foo`, `bar`, `baz`; set `${MPM_MPM_FILE}` to it; invoke `mpm __complete_source_names_in_mpm ""`; assert stdout equals `bar\nbaz\nfoo\n` (sorted); assert `completion-errors.log` is empty. Then truncate the fixture; re-invoke; assert stdout empty and a log entry appears. Recorded in `## TDD Cycle Log`.
- [ ] AC-DOC-001 `docs/shell-completion.md` (ref) updated in the same commit with the `__complete_source_names_in_mpm` row including the normalization-is-lossy callout.
...
| File | Change |
|------|--------|
| `src/mpm_cli/completions/source_names.py` | New -- `complete(current_token: str) -> list[str]` reading `${MPM_MPM_FILE}`. |
| `src/mpm_cli/cli.py` | Update -- register hidden `__complete_source_names_in_mpm` subcommand. |
| `tests/unit/test_complete_source_names.py` | New -- parametrized cases for happy path, missing file, malformed file, prefix filter, `MPM_COMPLETION_ENABLED=0`. |
| `tests/integration/test_complete_source_names.py` | New -- subprocess test against a real `.mpm` fixture. |
...
[2026-05-11 17:21 UTC] [agent/operator] [WU_WIRED] E7-F2-S1-T2 manually blocked on E7-F2-S1-T1 via `workbench add-dep`. [BLOCKED_PENDING_PROPOSAL] E7-F2-S1-T1
